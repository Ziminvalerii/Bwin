...
class PeerConnectivityManager:NSObject{
    ...
    var parentVC: CreateGameViewController?
    ...
    func presentMCBrowser(from vc: CreateGameViewController) {
    ...
    func send(message: PeerConnectionMessage) {
        guard let mcSession = mcSession else {return}
    ...
    func startAdvertisingPeer(parentVC: CreateGameViewController) {
...
extension PeerConnectivityManager: MCSessionDelegate {
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        do {
            let message = try JSONDecoder().decode(PeerConnectionMessage.self, from: data)
            switch message {
            case .playing(let score) :
                recieveDelegate?.recieved(message: .playing(score))
            case .gameOver:
                recieveDelegate?.recieved(message: .gameOver)
            default:
                return
            }
        } catch {
...
extension PeerConnectivityManager: MCNearbyServiceAdvertiserDelegate {
    func advertiser(_ advertiser: MCNearbyServiceAdvertiser, didReceiveInvitationFromPeer peerID: MCPeerID, withContext context: Data?, invitationHandler: @escaping (Bool, MCSession?) -> Void) {
        guard let conectData = try? JSONDecoder().decode(PeerConnectionMessage.self, from: context!) else {return}
        switch conectData {
        case .connecting(let model):
            if model.stake <= UserDefaultsValues.coinsCount {
                invitationHandler(true, mcSession)
                UserDefaultsValues.stake = model.stake
...
extension PeerConnectivityManager: MCBrowserViewControllerDelegate {
...
extension PeerConnectivityManager: MCNearbyServiceBrowserDelegate {
    func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String : String]?) {
        ...
        let model = PeerConnectionMessage.connecting(ConnectingModel(stake: stake))
